...
public class IdGeneratorTest {
	...
	public void testThreadSafetyTradeIDs() throws InterruptedException {
		...
		log.info("generated " + IdGenerator.TRADE_ID_START_VALUE + NUMBER_OF_THREADS* NUMBER_OF_ID_REQUESTS + " Trade IDs in " + finalTime + "ms with " + NUMBER_OF_THREADS + " Threads" );
	...
	public void testThreadSafetyOrderIDs() throws InterruptedException {
		...
		// threads times their requests plus the starting value should equals the current ID
		long finalTime = Calendar.getInstance().getTimeInMillis() - startTime;
		log.info("generated " + IdGenerator.TRADE_ID_START_VALUE + NUMBER_OF_THREADS* NUMBER_OF_ID_REQUESTS + " Order IDs in " + finalTime + "ms with " + NUMBER_OF_THREADS + " Threads" );
